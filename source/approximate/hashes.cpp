...
unsigned long long hashstr(std::string algo, std::string str, unsigned int bit_limit, unsigned long long seed1, unsigned long long seed2){
    ...
    switch(algos[algo]){
        ...
        case 9:
            ...
            break;
        default:
            std::cerr << "Undefined hashing algorithm." << std::endl;
            break;
    }
    if(bit_limit != 0){
...
// unsigned long get_pow(unsigned long val, unsigned long pow){
//     static std::unordered_map<unsigned long, unsigned long> powers;
//     unsigned long retVal = 0;
//     if(powers.find(val) != powers.end()){
//         retVal = powers[val];
//     } else {
//         retVal = std::pow(val,pow);
//         powers[val] = pow;
//     }
//     return retVal;
// }
